...
class CausalConv3d(nn.Conv3d):
    ...
    def forward(self, x, cache_x=None):
        ...
        #print(f"in conv1 x shape: {x.shape}")
        if cache_x is not None and self._padding[4] > 0:
            cache_x = cache_x.to(x.device)
            x = torch.cat([cache_x, x], dim=2)
            padding[4] -= cache_x.shape[2]
        x = F.pad(x, padding)
        #print(f"x device in CausalConv3d forward: {x.device}")
...
class Encoder3d(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        if feat_cache is not None:
            ...
            cache_x = x[:, :, -CACHE_T:, :, :].clone()
            if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                #print(f"use cache for encoder conv1 at idx {idx}")
                cache_x = torch.cat(
                    [
                        feat_cache[idx][:, :, -1, :, :].unsqueeze(2).to(
                            cache_x.device),
            ...
            conv1_device = next(self.conv1.parameters()).device
            #print(f"device check x: {x.device} conv1: {conv1_device} ")
